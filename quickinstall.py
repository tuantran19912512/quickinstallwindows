...
def tim_o_dia_luu_tru_an_toan():
    ...
    o_dia_toi_uu = None; dung_luong_lon_nhat = 0
    for o_dia in danh_sach_o:
        ...
        try:
            ...
            if dung_luong_trong > 10 * 1024**3 and dung_luong_trong > dung_luong_lon_nhat:
                dung_luong_lon_nhat = dung_luong_trong; o_dia_toi_uu = o_dia
        except: pass
    if not o_dia_toi_uu: raise Exception("Nghiêm trọng: Không tìm thấy phân vùng lưu trữ nào (D, E...) trống trên 10GB!")
    return o_dia_toi_uu
...
def sao_luu_du_lieu_he_thong(thu_muc_dich, lua_chon_driver, lua_chon_wifi, ham_ghi_log):
    ...
    # 2. TÍCH HỢP CODE DỌN RÁC TRIỆT ĐỂ & TỰ HỦY
    kich_ban_setup_complete += (
        ':: =========================================================\n'
        ...
        ':: TU HUY KICH BAN SAU KHI HOAN TAT\n'
        'del /f /q "%~f0" >nul 2>&1\n'
    )
    ...
    if lua_chon_driver != "Không Backup Driver":
        ham_ghi_log(f"Kích hoạt mô-đun trích xuất trình điều khiển ({lua_chon_driver})...")
...
def tiem_kich_ban_winre(o_dia_luu_wim, ham_ghi_log):
    ...
    ham_ghi_log(f"Đã cấp phát tên máy chủ định danh: {ten_may_chong_trung} (Phòng tránh xung đột IP)")
...
"@
    ...
    if ket_qua.returncode != 0: ham_ghi_log(f"Cảnh báo WinRE: Quá trình nạp gặp sự cố -> {ket_qua.stderr.strip()}")
...
def truat_xuat_du_lieu_dam_may(ma_file_tai, duong_dan_luu_tru, ham_cap_nhat_giao_dien, ham_ghi_log, su_kien_huy):
    for thu_tu, khoa_bao_mat_b64 in enumerate(DANH_SACH_KHOA_API):
        ...
        yeu_cau_ket_noi = urllib.request.Request(duong_dan_truy_xuat, headers={'User-Agent': 'Mozilla/5.0'})
        try:
            ...
            ham_ghi_log(f"Khóa API nhánh {thu_tu + 1} sự cố, chuyển nhánh...")
            continue
...
class BangDieuKhienTrungTam(ctk.CTk):
    def __init__(self):
        ...
        self.khung_thiet_lap = ctk.CTkFrame(self); self.khung_thiet_lap.grid(row=1, column=0, padx=20, pady=5, sticky="ew")
        ...
        self.khung_phan_cuoi = ctk.CTkFrame(self, fg_color="transparent"); self.khung_phan_cuoi.grid(row=4, column=0, padx=20, pady=15, sticky="ew")
    ...
    def tien_hanh_quet_kho_du_lieu(self):
        def luong_xu_ly_mang():
            try:
                yeu_cau_csv = urllib.request.Request(DUONG_DAN_KHO_DU_LIEU, headers={'User-Agent': 'Mozilla/5.0'}); phan_hoi_csv = urllib.request.urlopen(yeu_cau_csv); du_lieu_chuoi = phan_hoi_csv.read().decode('utf-8').splitlines(); trinh_doc_csv = csv.DictReader(du_lieu_chuoi)
                cac_cot_tieu_de = trinh_doc_csv.fieldnames
                khoa_ten_file = 'Filename' if 'Filename' in cac_cot_tieu_de else 'Name'; khoa_ma_file = 'ID' if 'ID' in cac_cot_tieu_de else 'FileID'
                for hang_du_lieu in trinh_doc_csv:
                    ten_hien_thi = hang_du_lieu.get(khoa_ten_file, ''); ma_dinh_danh = hang_du_lieu.get(khoa_ma_file, '')
                    if ten_hien_thi and ".wim" in ten_hien_thi.lower(): self.sinh_nut_cai_dat(ten_hien_thi, ma_dinh_danh)
    ...
    def khoi_tao_tien_trinh(self, nhan_ten_ban_cai, gia_tri_ma_file):
        ...
        if not self.bien_an_toan_test.get():
            if not messagebox.askyesno("Xác Nhận Nguy Hiểm", "Hành động này sẽ XÓA SẠCH ổ C. Chắc chắn tiếp tục?"): return
    ...
    def luong_dieu_phoi_chinh(self, nhan_ten_ban_cai, gia_tri_ma_file):
        try:
            ...
            if toc_do_mang_hien_tai < 1.5:
                if messagebox.askyesno("Báo Cáo Đường Truyền", f"Tốc độ mạng quá chậm (~{toc_do_mang_hien_tai:.1f}MB/s). Tải file wim từ ổ cứng?"):
            ...
            if duong_dan_usb_noi_bo:
                self.in_nhat_ky_he_thong("Đang di chuyển tập tin WIM..."); shutil.copy2(duong_dan_usb_noi_bo, vi_tri_luu_file_wim)
            else:
                self.in_nhat_ky_he_thong(f"Mở luồng tải dữ liệu đám mây: {nhan_ten_ban_cai}...")
                if not truat_xuat_du_lieu_dam_may(gia_tri_ma_file, vi_tri_luu_file_wim, self.lam_moi_giao_dien_tai, self.in_nhat_ky_he_thong, self.su_kien_huy_lenh): 
                    return self.khoi_phuc_trang_thai_goc("Tiến trình tải dữ liệu bị hủy.")

            if self.bien_an_toan_test.get():
                messagebox.showinfo("Thành Công", f"Đã lưu tại: {vi_tri_luu_file_wim}"); return self.khoi_phuc_trang_thai_goc("Kiểm thử xong.")
...
if __name__ == "__main__":
    ung_dung_ky_thuat = BangDieuKhienTrungTam(); ung_dung_ky_thuat.mainloop()import os
import sys
import time
import ctypes
import string
